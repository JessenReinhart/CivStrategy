...
/** Enemy AI with a resilient town-building director while preserving legacy combat/economy behavior. */
export class ProactiveEnemyAISystem extends EnemyAISystem {
  ...
  private tickTownBuild(): void {
    ...
    for (let i = 0; i < this.buildings.length; i++) {
      ...
      if (existing && (!existing.scene || existing.getData('hp') <= 0)) this.buildings[i] = null;
    }
  ...
  private syncBuildIndex(
    ...
    this.buildIndex = firstMissingUnlocked >= 0 ? firstMissingUnlocked : Math.max(0, blueprintLength - 1);
  }
  ...
  private tryTownExpansion(age: Age, personality: string): void {
    ...
    for (let step = 0; step < pool.length; step++) {
      ...
      const site = this.findBuildSite(type, this.baseX + Math.cos(angle) * ring, this.baseY + Math.sin(angle) * ring);
      if (!site) continue;
  ...
  private getExpansionPool(age: Age, personality: string): BuildingType[] {
    ...
    return (byPersonality[personality] ?? byPersonality.balanced).filter(type => isBuildingUnlockedForAI(type, age));
  }
  ...
  private findBuildSite(type: BuildingType, desiredX: number, desiredY: number): { x: number; y: number } | null {
    ...
    for (const offset of generateBuildSearchOffsets()) {
      ...
      if (!this.isBuildableTerrain(x, y, def) || this.overlapsExistingBuilding(x, y, def)) continue;
  ...
  private isBuildableTerrain(x: number, y: number, def: BuildingDef): boolean {
    if (this.mainScene.mapMode === MapMode.FIXED) {
      ...
      if (x < halfW || y < halfH || x > this.mainScene.mapWidth - halfW || y > this.mainScene.mapHeight - halfH) return false;
    }
    ...
    return terrain.getSlopeAt(x, y).slope <= TERRAIN_CONFIG.MAX_BUILDABLE_SLOPE;
  }
