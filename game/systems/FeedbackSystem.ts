...
export class FeedbackSystem {
    ...
    private readonly notificationHandler: (data: { text: string; severity?: Notification['severity']; duration?: number }) => void;
    ...
    constructor(scene: MainScene) {
        ...
        this.scene.events.on(EVENTS.NOTIFICATION, this.notificationHandler);
    }

    /** Release scene listeners and pooled display objects when the system is discarded. */
    destroy(): void {
        if (this.destroyed) return;
        this.destroyed = true;
        this.scene.events.off(EVENTS.NOTIFICATION, this.notificationHandler);
        ...
        for (const pool of pools) {
            for (const object of pool) object.destroy();
            pool.length = 0;
        }
    }
    ...
    notifyResearchComplete(techName: string): void { this.addNotification(`Research complete: ${techName}`, 'success'); }
    notifyEnemyApproaching(): void { this.addNotification('Enemy forces are approaching!', 'danger'); }
    notifyAnimalDepleted(species: string): void { this.addNotification(`${species} population depleted`, 'warning'); }
    notifySeasonChanged(season: string): void { this.addNotification(`Season changed to ${season}`, 'info'); }
    notifyHappinessCritical(): void { this.addNotification('Happiness critically low — revolt risk!', 'danger'); }
    ...
    private acquireText(): Phaser.GameObjects.Text | null {
        ...
        const text = this.textPool.pop() ?? this.scene.add.text(0, 0, '', { fontFamily: 'Arial', fontSize: '16px', fontStyle: 'bold', stroke: '#000000', strokeThickness: 4 });
        this.activeText++;
        text.setActive(true).setVisible(true).setAlpha(1).setScale(1);
        return text;
    }
    private releaseText(text: Phaser.GameObjects.Text): void { text.setActive(false).setVisible(false).setAlpha(0); this.activeText = Math.max(0, this.activeText - 1); if (this.textPool.length < FeedbackSystem.MAX_POOL_TEXT) this.textPool.push(text); }
    private acquireSpark(): Phaser.GameObjects.Arc | null {
        ...
        this.activeSpark++; spark.setActive(true).setVisible(true).setAlpha(1).setScale(1); return spark;
    }
    private releaseSpark(spark: Phaser.GameObjects.Arc): void { spark.setActive(false).setVisible(false).setAlpha(0); this.activeSpark = Math.max(0, this.activeSpark - 1); if (this.sparkPool.length < FeedbackSystem.MAX_POOL_SPARK) this.sparkPool.push(spark); }
    private acquireFloatingText(): Phaser.GameObjects.Text | null {
        ...
        const text = this.floatingTextPool.pop() ?? this.scene.add.text(0, 0, '', { fontFamily: 'Arial', fontSize: '14px', stroke: '#000000', strokeThickness: 3 });
        this.activeFloatingText++; text.setActive(true).setVisible(true).setAlpha(1).setScale(1); return text;
    }
    private releaseFloatingText(text: Phaser.GameObjects.Text): void { text.setActive(false).setVisible(false).setAlpha(0); this.activeFloatingText = Math.max(0, this.activeFloatingText - 1); if (this.floatingTextPool.length < FeedbackSystem.MAX_POOL_FLOATING_TEXT) this.floatingTextPool.push(text); }
    private acquireDeathRing(): Phaser.GameObjects.Arc | null {
        ...
        this.activeDeathRing++; ring.setActive(true).setVisible(true).setAlpha(0.6).setScale(1); return ring;
    }
    private releaseDeathRing(ring: Phaser.GameObjects.Arc): void { ring.setActive(false).setVisible(false).setAlpha(0); this.activeDeathRing = Math.max(0, this.activeDeathRing - 1); if (this.deathRingPool.length < FeedbackSystem.MAX_POOL_DEATH_RING) this.deathRingPool.push(ring); }
    private acquireDeathFlash(): Phaser.GameObjects.Arc | null {
        ...
        this.activeDeathFlash++; flash.setActive(true).setVisible(true).setAlpha(0.8).setScale(1); return flash;
    }
    private releaseDeathFlash(flash: Phaser.GameObjects.Arc): void { flash.setActive(false).setVisible(false).setAlpha(0); this.activeDeathFlash = Math.max(0, this.activeDeathFlash - 1); if (this.deathFlashPool.length < FeedbackSystem.MAX_POOL_DEATH_FLASH) this.deathFlashPool.push(flash); }

    // ── Damage feedback ──────────────────────────────────────────────────

    showDamageNumber(x: number, y: number, damage: number, damageType?: string): void {
        const iso = toIso(x, y); const colorMap: Record<string, string> = { Hack: '#ef4444', Pierce: '#f97316', Crush: '#fbbf24' }; const text = this.acquireText(); if (!text) return;
        text.setText(`-${damage}`).setColor(damageType ? (colorMap[damageType] || '#ef4444') : '#ef4444'); text.setPosition(iso.x + Phaser.Math.Between(-15, 15), iso.y - 30).setOrigin(0.5).setDepth(Number.MAX_VALUE);
    ...
    showHitSpark(x: number, y: number, damageType?: string): void {
        const iso = toIso(x, y); const colorMap: Record<string, number> = { Hack: 0xff4444, Pierce: 0xff8800, Crush: 0xffcc00 }; const color = damageType ? (colorMap[damageType] ?? 0xff4444) : 0xffffff;
        for (let i = 0; i < 3; i++) {
            const spark = this.acquireSpark(); if (!spark) break; spark.setFillStyle(color, 1).setRadius(Phaser.Math.Between(2, 4));
    ...
    showFloatingText(x: number, y: number, message: string, color: string = '#ffffff'): void {
        const iso = toIso(x, y); const text = this.acquireFloatingText(); if (!text) return;
        text.setText(message).setColor(color); text.setPosition(iso.x, iso.y - 50).setOrigin(0.5).setDepth(Number.MAX_VALUE);
        this.scene.tweens.add({ targets: text, y: iso.y - 100, alpha: 0, duration: 1500, onComplete: () => this.releaseFloatingText(text) });
    }
    showFloatingResource(x: number, y: number, amount: number, type: string): void { const colorMap: Record<string, string> = { Wood: '#4ade80', Food: '#facc15', Gold: '#fbbf24' }; this.showFloatingText(x, y, `+${amount} ${type}`, colorMap[type] || '#ffffff'); }

    showDeathEffect(x: number, y: number, color: number = 0xff4444): void {
        const iso = toIso(x, y); const ring = this.acquireDeathRing();
        if (ring) {
            ring.setFillStyle(color, 0.6).setPosition(iso.x, iso.y).setScale(1, 1).setDepth(Number.MAX_VALUE - 1);
            this.scene.tweens.add({ targets: ring, scaleX: 2.5, scaleY: 1.5, alpha: 0, duration: 500, onComplete: () => this.releaseDeathRing(ring) });
        }
