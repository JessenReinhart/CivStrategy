...
const readRuntimeProbe = () => page.evaluate(() => {
  ...
  return {
    ...
    player: player ? {
      ...
      visual: player.visual ? { x: player.visual.x, y: player.visual.y } : null,
      velocity: player.body ? { x: player.body.velocity.x, y: player.body.velocity.y } : null,
    } : null,
    enemy: enemy ? {
      x: enemy.x,
      y: enemy.y,
...
try {
  ...
  telemetry.setup = await page.evaluate(() => {
    ...
    if (!existing) throw new Error('No player military unit available to anchor army journey.');
  ...
  const movePoint = await cartesianScreenPoint({ x: telemetry.setup.arena.moveX, y: telemetry.setup.arena.moveY });
  ...
  await page.waitForFunction(() => {
    const visual = window.__armyCombatProbe?.enemy?.visual;
    return Boolean(visual && Number.isFinite(visual.x) && Number.isFinite(visual.y));
  }, undefined, { timeout: 5_000 });
  await page.evaluate(() => {
    const scene = window.__civStrategyGame.scene.getScene('MainScene');
